...
size_t Dci_Strftime(char *s, size_t maxs, const char *f, time_t time)
{
      int w, dn, yr, mo, dy;
      char *p, *q, *r;
      char buf[26];
      ...
      dn = Dci_Time2Dn(time);
      Dci_Dn2Date(dn, &yr, &mo, &dy);
      tt.tm_sec = time % 60;
      tt.tm_hour = (time / 3600) % 24;
      tt.tm_min = ((time / 60) % (24 * 60)) - (tt.tm_hour * 60);
      tt.tm_year = yr - 1900;
      tt.tm_mon = mo - 1;
      tt.tm_mday = dy;
      tt.tm_wday = Dci_Dn2Dow(dn);
      tt.tm_yday = dn - Dci_Date2Dn(yr, 1, 1);
      tt.tm_isdst = 0;
      t = &tt;
